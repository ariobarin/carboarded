...
def test_collision_penalty_skipped_when_wall_contact_enabled():
    """When wall_contact_penalty is set, the one-shot collision_penalty is not applied."""
    env = _make_env(wall_contact_penalty=-0.5, collision_penalty=-20.0)
    env.car.collided = True

    _, reward, _, _, _ = env.step(np.array([0.0, 0.0], dtype=np.float32))

    # With wall_contact_penalty != 0, collision_penalty (-20) should be skipped
    baseline_env = _make_env(wall_contact_penalty=-0.5, collision_penalty=-20.0)
    _, baseline_reward, _, _, _ = baseline_env.step(np.array([0.0, 0.0], dtype=np.float32))

    # Both should be same since touching_wall is False (no actual contact) and
    # collision_penalty is skipped when wall_contact_penalty is set
    assert abs(reward - baseline_reward) < 1e-5
